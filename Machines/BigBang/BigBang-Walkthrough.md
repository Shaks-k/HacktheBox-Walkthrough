...
But when target's libc.so.6 is downloaded using the lfi, it is corrupted, so its equivalent can be used that is local libc.so.6 on your machine.
Check the version of target's libc , it is 2.3.6 so get its local equivalent.
